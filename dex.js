...
/** Utilities */
//NO ENCONTRÉ. Converting from Wei using custom Function
...
async function formSubmitted(event) {
    ...
    try {
        const quote = await Moralis.Plugins.oneInch.quote({
          ...
          document.querySelector.apply('.js-quote-container').innerHTML = `
            <p>${fromAmount} ${quote.fromToken.symbol} = ${toAmount} ${quote.toToken.symbol}</p>
        `;

    } catch(e) {
        document.querySelector.apply('.js-quote-container').innerHTML = `
            <p class="error">The conversion didnt succeed ERRORROROR</p>
        `;
    }
...
// CAMBIAR POR EL DE MORALIS QUE FUNCIONA! APIII
